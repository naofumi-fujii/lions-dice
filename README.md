...
- **✏️ 6面のテーマを編集** から 6 面のテキストを自由に書き換え（`localStorage` に自動保存）
  - 初回のみ、この機能に気づけるようボタンの上に吹き出しを表示します
    （閉じるか一度開くと `localStorage` に記録して以降は出しません）
- **✏️ 6面のテーマを編集 → JSON からまとめて読み込む** で、外部 JSON のテーマ一覧を取り込み、
